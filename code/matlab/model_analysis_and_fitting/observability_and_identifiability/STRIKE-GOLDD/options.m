...
modelname ='UVA_cprc_wt_ca';    % Name of a .mat file placed in the 'models' folder 
% modelname ='UVA_cprc_ko_ca';    % Name of a .mat file placed in the 'models' folder 
% modelname ='UVA_innos_ca';    % Name of a .mat file placed in the 'models' folder 
% modelname ='UVA_innos_no';    % Name of a .mat file placed in the 'models' folder 
...
% prev_ident_pars = [delta_C delta_2 delta_1 kappa_UV delta_S1 u2_0 ...
%                    kappa_4 delta_4 kappa_5_1 delta_5 kappa_5_2];
...
opts.affine_workers      = 12;     % number of workers for parallel pool.
opts.affine_graphics     = 0;     % display graphics (=1) or not (=0)
...
opts.multiexp_user_nnzDerU = 1;     % Set manually the number of non-zero known input derivatives in each experiment (=1) or not (=0).
opts.multiexp_nnzDerU      = [1 1]; % Number of non-zero known input derivatives in each experiment (Rows=inputs;Columns=experiments).
opts.multiexp_user_nnzDerW = 0;     % Set manually the number of non-zero unknown input derivatives in each experiment (=1) or not (=0).
opts.multiexp_nnzDerW      = [0 0]; % Number of non-zero unknown input derivatives in each experiment (Rows=inputs;Columns=experiments).
...
opts.results_file = 'obs_ident_matrix_UVA_3outputs_10_Lie_deriv.mat'; % .mat file to use if use_existing_results = 1                              
